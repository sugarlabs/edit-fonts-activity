...
from gi.repository import WebKit, GLib, GdkPixbuf # noqa

from widgets.localIcon import * # noqa
...
class ManagerPage(Gtk.Box):
    ...
    def __init__(self, activity): # noqa
        super(ManagerPage, self).__init__()
        self.activity = activity

        self._init_ui()

    def update(self, activity):  # noqa
        # FIXME: this shouldn't destroy anything
    ...
    def init_fonts(self):  # noqa
        ...
        (__, __, active_fonts_path) = os.walk(active_fonts_file_path).next()

        # get all files in the folder
        active_fonts_path = [val.strip('.ttf') for i, val in
                             enumerate(active_fonts_path) if
                             val.endswith('.ttf')]
        ...
        # get all files in the folder
        (__, __, inactive_fonts_path) = os.walk(inactive_fonts_file_path)\
            .next()
        # Favorite Fonts
        inactive_fonts_path = [val.strip('.ttf') for i, val in
                               enumerate(inactive_fonts_path) if
                               val.endswith('.ttf')]
...
class FontsTreeView(Gtk.TreeView):  # noqa
    """
    This creates the free view which contains
    row containing the fonts and there and samples
    """
    ...
    def __init__(self):  # noqa
        ...
        # self.xo_color = XoColor(client.get_string('/desktop/sugar/user/color')) # noqa
    ...
    def on_treeview_selection_changed(self, selection):  # noqa
    ...
    def __activate_set_data_cb(self, column, cell, model, tree_iter, data):
        # DOUBT: Currently I'm showing the a green icon when the font is
        # active and a red icon when its deactivated so I'm using the this
        # cell as a status indicator rather than a button with when shows
        # a green tick sign means the cell will be activated if the button
        # is clicked - is this setting correct?
    ...
    def __activate_clicked_cb(self, cell, path):
        ...
        if is_activated:
            ...
            batcmd = "mv " + active_fonts_file_path + "/" + font_name +\
                ".ttf " + inactive_fonts_file_path + "/" + font_name +\
                ".ttf "
            # result = subprocess.check_output(batcmd, shell=True)
            batcmd = "fc-cache -f -v"
            # result = subprocess.check_output(batcmd, shell=True)
    ...
    def __favorite_clicked_cb(self, cell, path):  # noqa
        """
    ...
    def set_filter(self, query):  # noqa
...
class ListModel(Gtk.ListStore):  # noqa
    ...
    def __init__(self): # noqa
        ...
        for font_name in _all_system_fonts:
            ...
            data = [favorite, font_name,
                    'The quick brown fox jumps over the lazy dog', 1.7, 1,
                    True, -1, False]
...
class CellRendererClickablePixbuf(Gtk.CellRendererPixbuf): # noqa
    ...
    def do_activate(self, event, widget, path, background_area, cell_area,  # noqa
                    flags):
        self.emit('clicked', path)


class FontsList(Gtk.VBox):  # noqa
    ...
    def __init__(self):  # noqa
        ...
        # creating the treeview, making it use the filter as a model, and adding the columns  # noqa
    ...
    def font_name_filter_func(self, model, iter, data):  # noqa
        """Tests if the language in the row is the one in the filter"""
        """
        if self.current_filter_query is None or\
                self.current_filter_query == "None":
            return True
        else:
    ...
    def _search(self, widget, event, entry): # noqa
