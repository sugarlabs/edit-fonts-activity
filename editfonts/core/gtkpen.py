...
class GtkPen(BasePen):
    ...
    def X(self, x):
        t = self.pos + float(x) *\
            globals.GLYPH_BOX[self.id]['height'] / self.h

        """
        H = globals.GLYPH_BOX[self.id]['EDITOR_BOX_HEIGHT']
        W = globals.GLYPH_BOX[self.id]['EDITOR_BOX_WIDTH']
    ...
    def Y(self, y):
        ...
        t = float(1.1 * self.h - y - self.b) *\
            globals.GLYPH_BOX[self.id]['height'] / 1.2 * self.h
    ...
    def _curveToOne(self, p1, p2, p3):
        ...
        x2, y2 = p2
        x3, y3 = p3
        self.cr.curve_to(self.X(x1), self.Y(y1),
                         self.X(x2), self.Y(y2),
                         self.X(x3), self.Y(y3))
