...
class DragPoint(Gtk.EventBox):
    ...
    def is_appearance_for(self, point):
        self.point = point
        self.set_x(globals.X(point.x))
        self.set_y(globals.Y(point.y))
    ...
    def update_connected_points(self, dx, dy):
        ...
        if self.point.segmentType is None:
            ...
            B.update(O.get_x() + (O.get_x() - self.get_x()) * r / d,
                     O.get_y() + (O.get_y() - self.get_y()) * r / d)
